...
void print_help(const char *program_name) {
    ...
    printf(" -d, --daemon-exec  Used with -e: executes commands in background as daemons (via fork + setsid). "
           "Output from executed commands is written to the file specified by the environment variable "
           "gorgona_LOG_FILE (e.g., gorgona_LOG_FILE=/var/log/gorgona.log ./gorgona -ed listen new ...). "
           "If gorgona_LOG_FILE is not set, output is discarded (/dev/null).\n");
